    ...
    <main className="min-h-screen bg-black text-white p-4 flex flex-col items-center">
      ...
      {fase === 'lobby' && (
        <>
          ...
          <ul className="bg-gray-800 border border-gray-700 rounded-xl p-4 mb-6 w-full max-w-md">
            {jugadores.map((j, i) => (
          ...
          {jugadores.length >= 2 && (
            <button
              className="bg-orange-500 text-black px-4 py-2 rounded-lg shadow"
      ...
      <div className="w-full max-w-2xl mt-10 bg-gray-900 border border-gray-700 rounded-lg shadow-md">
        <div
          ref={chatRef}
          className="h-64 overflow-y-auto px-4 py-3 space-y-2 text-sm"
        >
          {mensajes.map((m, i) => (
            <div key={i}>
              <span className="text-orange-400 font-semibold">{m.nombre}:</span>{' '}
              <span>{m.mensaje}</span>
            </div>
          ))}
        </div>
        <div className="flex border-t border-gray-700">
          ...
          <button
            onClick={async () => {
              ...
              await supabase.from('chat').insert([{ sala: codigo, nombre: nombreJugador, mensaje: mensajeNuevo.trim() }]);
              setMensajeNuevo('');
              setTimeout(() => {
                if (chatRef.current) {
            ...
            className="px-4 bg-orange-500 hover:bg-orange-600 text-black font-bold"
          >
            Enviar
          </button>
        </div>
